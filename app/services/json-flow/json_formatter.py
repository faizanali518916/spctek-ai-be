...
def _cover(report: dict, story: list):
    ...
    # Navy banner block built as a single-cell table
    ...
    def _pct_cell(pct, label):
        return [
            Paragraph(pct, S["pct_big"]),
            Paragraph(label, S["pct_label"]),
        ]
    ...
    if ctx:
        story.append(
            CalloutBox(
                [Paragraph("<b>Suspension Context</b>", S["callout_label"]), Paragraph(ctx, S["callout_body"])],
            )
        )
...
def _section_root_cause(rc: dict, story: list):
    ...
    badge_style = ParagraphStyle(
        "badge",
        parent=S["confidence_badge"],
        textColor=txt_color,
    )
    badge = Table(
        [[Paragraph(f"AI Confidence: {confidence}", badge_style)]],
        colWidths=[1.6 * inch],
    )
    badge.setStyle(
        TableStyle(
...
def _section_chances(chances: dict, story: list):
    ...
    if ci:
        ...
        # Convert plain strings in rows to Paragraphs
        fmt_rows = []
        for r in rows:
            fmt_rows.append(
                [
...
def _section_steps(steps: list, story: list):
    ...
    for step in steps:
        ...
        header_row = Table(
            [[badge_cell, Paragraph(title, S["step_title"])]],
            colWidths=[0.38 * inch, PAGE_W - 0.38 * inch],
        )
        header_row.setStyle(
            TableStyle(
                [
...
def _section_poa(poa: dict, story: list):
    ...
    if template:
        ...
        story.append(
            CalloutBox(
                [Paragraph(line, S["callout_body"]) for line in template.split("\n") if line.strip()],
                bar_color=GOLD,
...
def _section_summary(fs: dict, story: list):
    ...
    if conclusion:
        story.append(
            CalloutBox(
                [Paragraph(conclusion, S["callout_body"])],
                bar_color=NAVY,
                bg_color=GREY_LIGHT,
            )
        )
...
def write_formatted_report(report: dict, filename: str = "report.pdf"):
    """
    Render *report* (the parsed JSON dict from the LLM) into a PDF at *filename*.

    Args:
        report:   Parsed reinstatement report dict (the value of report["report"]).
        filename: Output PDF path.
    """
    # Accept either the full envelope {"report": {...}} or the inner dict directly
    if "report" in report:
